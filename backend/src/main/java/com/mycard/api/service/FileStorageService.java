...
public class FileStorageService {
    ...
    public Resource loadFileAsResource(Long attachmentId, UserPrincipal currentUser) {
        ...
        if (!attachment.isAccessibleBy(currentUser.getId()) && !ownerCheckService.isAdminOrOperator(currentUser)) {
            throw new com.mycard.api.exception.AccessDeniedException("이 파일에 접근할 권한이 없습니다.");
        }
    ...
    public void deleteFile(Long attachmentId, UserPrincipal currentUser) {
        ...
        if (!attachment.getUploadedBy().getId().equals(currentUser.getId())
                && !currentUser.isAdmin()) {
            throw new com.mycard.api.exception.AccessDeniedException("이 파일을 삭제할 권한이 없습니다.");
        }
    ...
    private void validateFile(MultipartFile file) {
        ...
        if (file.getSize() > maxFileSize) {
            throw new BadRequestException("파일 크기가 허용 범위를 초과했습니다. (최대 " + (maxFileSize / 1024 / 1024) + "MB)");
        }
        ...
        if (!allowed.contains(extension)) {
            throw new BadRequestException("허용되지 않는 파일 형식입니다. 허용: " + allowedExtensions);
        }
